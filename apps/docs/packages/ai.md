...
These functions do not read environment variables, store keys, send requests or perform retries. Output-budget state reports when to stop; it does not increase the budget or initiate another call. See the [package README](https://github.com/gaopengbin/openstyle/tree/main/packages/ai) for exact behavior and examples. New policy helpers are available in local source pending a separate package release.
